...
class OpenAIClient(BaseLLMClient):
    ...
    def chat(
        ...
        # If the asyncio task was cancelled while this thread was blocked on the
        # HTTP call, skip all state mutations and trajectory recording.  This
        # prevents message_history from being corrupted with data from an
        # interrupted turn and avoids writing a spurious trajectory file.
        ...
        # Persist the incoming delta to message_history so subsequent steps see the
        # full conversation (tool results from this step must be visible next step).
        if reuse_history:
