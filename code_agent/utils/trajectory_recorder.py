...
"""Trajectory recording functionality for OpenCook."""
...
class TrajectoryRecorder:
    ...
    def start_recording_first(self, task: dict) -> None:
        ...
        self.trajectory_data.update(
            {
                "task": task,
                "start_time": self._start_time.isoformat() if len(self.trajectory_data.get("start_time", "")) == 0
                else self.trajectory_data.get("start_time", ""),
            }
        )
        self.save_trajectory()

    def start_recording(self, agent_type: str, task: dict, provider: str, model: str, max_steps: int) -> None:
        """Start recording a new trajectory.
        ...
        self.trajectory_data.update(
            {
                ...
                "llm_interactions": [] or self.trajectory_data.get("llm_interactions", []),
                "agent_steps": [] or self.trajectory_data.get("agent_steps", []),
            }
        )
        self.save_trajectory()
    ...
    def finalize_recording(self, agent_type: str, success: bool, final_result: str | None = None) -> None:
        ...
        self.trajectory_data[agent_type].update(
            {
    ...
    def finalize_recording_last(self, success: bool, final_result: str | None = None) -> None:
        """Finalize the trajectory recording.
