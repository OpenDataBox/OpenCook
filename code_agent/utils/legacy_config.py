...
@dataclass
class LegacyConfig:
    """Configuration manager for OpenCook."""
    ...
    def __init__(self, config_or_config_file: str | dict = "opencook_config.json"):  # pyright: ignore[reportMissingTypeArgument, reportUnknownParameterType]
        ...
        # Legacy JSON configs predate per-agent run_steps. Default to one
        # planning pass so the modern subagent pipeline still produces a plan.
        self.run_steps = int(self._config.get("run_steps", 1))
