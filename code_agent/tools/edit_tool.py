...
from code_agent.utils.encoding_utils import candidate_encodings, path_key
from code_agent.tools.run import maybe_truncate
...
class TextEditorTool(Tool):
    ...
    def __init__(self, model_provider: str | None = None) -> None:
        ...
        self._file_newlines: dict[str, str] = {}
...
* Always obtain `old_str` from the most recent successful `view` output of this same tool. Do not reconstruct `old_str` from bash/PowerShell output, search output, memory, or paraphrases
* Do not use short anchors such as `};` or a single repeated line. Expand `old_str` to include enough surrounding unique context from the same `view` result
* Do not pass line numbers from bash/PowerShell output directly into `view_range` or `insert_line`. If line-number editing is needed, first get the line numbers from this tool's own `view` output
* If a replacement fails because `old_str` did not appear verbatim or was not unique, stop guessing. Fetch a fresh `view` around the target and retry once with a longer exact anchor
* After a successful `insert`, use `view` on the edited neighborhood before making the next edit, and fix any extra braces, duplicated lines, or malformed spacing immediately
"""
    ...
    async def _view(self, path: Path, view_range: list[int] | None = None) -> ToolExecResult:
        ...
        if path.is_dir():
            ...
            try:
                ...
                raise ToolError(f"Ran into {e} while trying to inspect directory {path}") from None
            stdout = (
    ...
    def _list_directory(self, path: Path, max_depth: int = 2) -> list[str]:
    ...
    def read_file(self, path: Path):
        ...
        try:
            raw = path.read_bytes()
            fkey = path_key(path)
            self._file_newlines[fkey] = self._detect_newline_style(raw)
            last_error: Exception | None = None
            for encoding in candidate_encodings():
                try:
                    text = raw.decode(encoding)
                    self._file_encodings[fkey] = encoding
                    return self._normalize_newlines(text)
                except UnicodeDecodeError as exc:
                    last_error = exc
            if last_error is not None:
                raise last_error
            return self._normalize_newlines(raw.decode("utf-8"))
        except Exception as e:
    ...
    def write_file(self, path: Path, file: str):
        ...
        try:
            fkey = path_key(path)
            encoding = self._file_encodings.get(fkey)
            newline = self._file_newlines.get(fkey)
            if encoding is None and path.exists():
            ...
            if newline is None:
                newline = "\n"
            normalized_file = self._normalize_newlines(file)
            encoded_text = normalized_file.replace("\n", newline)
            _ = path.write_bytes(encoded_text.encode(encoding))
        except Exception as e:
