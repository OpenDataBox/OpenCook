...
"""
        ...
        try:
            ...
            for encoding in candidate_encodings():
                try:
                    ...
                    self._file_encodings[path_key(file_path)] = encoding
            ...
            if last_error is not None:
                raise last_error
            raise ToolError(f"Could not decode JSON file: {file_path}")
    ...
    async def _save_json_file(
        ...
        try:
            encoding = self._file_encodings.get(path_key(file_path), "utf-8")
